...
void GrayscaleFilter::ProcessPartition(const std::vector<uint8_t>& image_data, std::vector<uint8_t>& grayscale_data,
                                       const int width, int height, const int start_y,
...
void GrayscaleFilter::RunThreads(std::vector<uint8_t>& image_data, std::vector<uint8_t>& grayscale_data,
                                 int width, const int height, int num_threads) const {
  ...
  for (int i = 0; i < num_threads; ++i) {
    ...
    threads.emplace_back(&GrayscaleFilter::ProcessPartition, this,
                     std::ref(image_data), std::ref(grayscale_data), width, height, start_y, end_y);
