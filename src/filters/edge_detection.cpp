...
void EdgeDetectionFilter::ProcessPartition(
    const std::vector<uint8_t> &image_data, std::vector<uint8_t> &edge_data,
...
void EdgeDetectionFilter::RunThreads(std::vector<uint8_t> &image_data,
  ...
  for (int i = 0; i < num_threads; ++i) {
    ...
    threads.emplace_back(&EdgeDetectionFilter::ProcessPartition, this,
                         std::ref(image_data), std::ref(sharpening_data), width,
